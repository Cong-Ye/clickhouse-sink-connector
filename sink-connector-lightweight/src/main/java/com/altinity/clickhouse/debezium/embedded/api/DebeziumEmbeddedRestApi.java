...
public class DebeziumEmbeddedRestApi {
    ...
    public static void startRestApi(Properties props, Injector injector,
        ...
        final MySQLDDLParserService sqlddlParserService = new MySQLDDLParserService(
        ...
        app = Javalin.create(javalinConfig -> {
            RoutesConfig routes = javalinConfig.routes;
            routes.get("/", ctx -> {
            ...
            // Delete offsets
            routes.delete("/schema-history", ctx -> {
                ClickHouseSinkConnectorConfig config =
                    new ClickHouseSinkConnectorConfig(
                        PropertiesHelper.toMap(finalProps1));
                String response = "";
            ...
            routes.get("/show-slave-status", ctx -> {
                String response = "";
                try {
                    ClickHouseSinkConnectorConfig config =
                        new ClickHouseSinkConnectorConfig(
        ...
        }).start(Integer.parseInt(cliPort));
